...
function renderProjectsPage() {
    return render(_jsx(MemoryRouter, { children: _jsx(AuthProvider, { children: _jsx(ProjectsPage, {}) }) }));
}
// ---------------------------------------------------------------------------
// Tests
// ---------------------------------------------------------------------------
describe('ProjectsPage', () => {
    ...
    it('calls POST /api/v1/projects with form values on submit', async () => {
        ...
        server.use(http.post('*/api/v1/projects', async ({ request }) => {
            ...
            return HttpResponse.json({ id: 'proj-new', name: postBody.name, description: postBody.description ?? '' }, { status: 201 });
        }));
        renderProjectsPage();
        await waitFor(() => screen.getByText(mockProject.name));
        await user.click(screen.getByRole('button', { name: /new project/i }));
    ...
    it('calls DELETE /api/v1/projects/:id when delete is confirmed', async () => {
        let deletedId = null;
        server.use(http.delete('*/api/v1/projects/:id', ({ params }) => {
            deletedId = params.id;
