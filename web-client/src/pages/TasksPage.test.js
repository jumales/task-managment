...
function renderTasksPage() {
    return render(_jsx(MemoryRouter, { children: _jsx(TasksPage, {}) }));
}
/**
 ...
 */
async function selectAntOption(labelText, optionText) {
    ...
    const option = await waitFor(() => {
        ...
        const match = Array.from(items).find((el) => el.textContent?.trim() === optionText);
        if (!match)
            throw new Error(`Option "${optionText}" not found for label "${labelText}"`);
        return match;
    });
...
describe('TasksPage', () => {
    ...
    it('shows validation errors when the form is submitted empty', async () => {
        ...
        server.use(http.get('*/api/v1/projects', () => HttpResponse.json([mockProject, { id: 'proj-2', name: 'Beta Project', description: '' }])));
        renderTasksPage();
        await waitFor(() => screen.getByText(mockTaskSummary.title));
        // Open modal
        await user.click(screen.getByRole('button', { name: /new task/i }));
    ...
    it('calls POST /api/v1/tasks with form values on submit', async () => {
        ...
        server.use(http.get('*/api/v1/projects', () => HttpResponse.json([mockProject, { ...mockProject, id: 'proj-2', name: 'Beta Project' }])));
        let postBody = null;
        server.use(http.post('*/api/v1/tasks', async ({ request }) => {
            postBody = (await request.json());
            return HttpResponse.json({ ...mockTaskSummary, id: 'task-new', title: postBody.title }, { status: 201 });
        }));
        renderTasksPage();
        await waitFor(() => screen.getByText(mockTaskSummary.title));
        await user.click(screen.getByRole('button', { name: /new task/i }));
    ...
    it('opens the Edit Task modal pre-filled when "Edit" is clicked', async () => {
        ...
        expect(screen.getByLabelText('Title').value).toBe(mockTaskSummary.title);
    });
    it('calls PUT /api/v1/tasks/:id when edit form is saved', async () => {
        const user = userEvent.setup();
        let putBody = null;
        server.use(http.put('*/api/v1/tasks/:id', async ({ request }) => {
            putBody = (await request.json());
            return HttpResponse.json({ ...mockTaskSummary, title: putBody.title });
        }));
        renderTasksPage();
        await waitFor(() => screen.getByText(mockTaskSummary.title));
        await user.click(screen.getByRole('button', { name: /^edit$/i }));
    ...
    it('calls DELETE /api/v1/tasks/:id when delete is confirmed', async () => {
        let deletedId = null;
        server.use(http.delete('*/api/v1/tasks/:id', ({ params }) => {
            deletedId = params.id;
        ...
        const confirmButton = await waitFor(() => {
            ...
            if (buttons.length < 2)
                throw new Error('Popconfirm not yet visible');
