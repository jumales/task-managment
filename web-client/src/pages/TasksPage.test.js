...
describe('TasksPage', () => {
    ...
    it('opens the Edit Task modal pre-filled when "Edit" is clicked', async () => {
        ...
        expect(screen.getByLabelText('Title').value).toBe(mockTask.title);
    });
    it('calls PUT /api/v1/tasks/:id when edit form is saved', async () => {
        ...
        server.use(http.put('*/api/v1/tasks/:id', async ({ request }) => {
            ...
            return HttpResponse.json({ ...mockTask, title: putBody.title });
        }));
        renderTasksPage();
        await waitFor(() => screen.getByText(mockTask.title));
        await user.click(screen.getByRole('button', { name: /^edit$/i }));
    ...
    it('calls POST /api/v1/tasks/:id/comments when a comment is submitted', async () => {
        ...
        server.use(http.post('*/api/v1/tasks/:taskId/comments', async ({ request }) => {
            commentBody = (await request.json());
            return HttpResponse.json({ id: 'c-1', content: commentBody.content, createdAt: '2026-03-23T10:00:00Z' }, { status: 201 });
        }));
        renderTasksPage();
        await waitFor(() => screen.getByText(mockTask.title));
        // Open detail drawer
