...
public class TaskController {
     ...
     * By default, tasks in RELEASED or REJECTED phases are excluded; pass {@code includeFinished=true} to include them.
     ...
     * @param includeFinished  when {@code true}, RELEASED and REJECTED tasks are included; defaults to {@code false}
     * @param pageable         pagination parameters (page, size, sort)
     */
    @Operation(summary = "List all tasks (paginated)",
               description = "Returns a page of tasks. By default, tasks in RELEASED or REJECTED phases are excluded. "
                           + "Pass `includeFinished=true` to include them. "
                           + "Optionally filter by `userId`, `projectId`, `status`, or `completionStatus`. "
                           + "`completionStatus=FINISHED` returns only RELEASED/REJECTED tasks; "
                           + "`completionStatus=DEV_FINISHED` returns only DONE tasks. "
    ...
    public PageResponse<TaskSummaryResponse> getAll(
            ...
            @Parameter(description = "When true, includes tasks in RELEASED and REJECTED phases (default: false)") @RequestParam(defaultValue = "false") boolean includeFinished,
            @PageableDefault(size = 20) Pageable pageable) {
        if (completionStatus != null) return service.findByCompletionStatus(completionStatus, pageable);
        if (userId != null) return service.findByUser(userId, includeFinished, pageable);
        if (projectId != null) return service.findByProject(projectId, includeFinished, pageable);
        if (status != null) return service.findByStatus(status, includeFinished, pageable);
        return service.findAll(includeFinished, pageable);
    }
